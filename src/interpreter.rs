...
"#;
    ...
    fn call_core_fn(&self, name: &str, args: Vec<Value>) -> Result<Value, String> {
        ...
        let req_list = |idx: usize, context: &str| -> Result<Rc<RefCell<Vec<Value>>>, String> {
            match args.get(idx) {
        ...
        let req_dict = |idx: usize, context: &str| -> Result<Rc<RefCell<IndexedMap>>, String> {
            match args.get(idx) {
                Some(Value::Dict(items)) => Ok(items.clone()),
        ...
        match name {
            ...
            "addr" => {
                ...
                Ok(Value::Int(core_runtime::addr(req_int(0, "core.addr")?)))
            }
            "addr_add" => {
                if args.len() != 2 {
                    return Err(self.err("core.addr_add() takes exactly two arguments"));
                }
                Ok(Value::Int(core_runtime::addr_add(
            ...
            "addr_sub" => {
                ...
                Ok(Value::Int(core_runtime::addr_sub(
            ...
            "addr_diff" => {
                ...
                Ok(Value::Int(core_runtime::addr_diff(
            ...
            "string_len" => {
                ...
                Ok(Value::Int(core_runtime::string_len(&req_str(0, "core.string_len")?)))
            }
            "string_repeat" => {
                if args.len() != 2 {
                    return Err(self.err("core.string_repeat() takes exactly two arguments"));
                }
                Ok(Value::Str(core_runtime::string_repeat(
            ...
            "format_hex" => {
                ...
                Ok(Value::Str(core_runtime::format_hex(req_int(0, "core.format_hex")?)))
            }
            "format_bin" => {
                if args.len() != 1 {
                    return Err(self.err("core.format_bin() takes exactly one argument"));
                }
                Ok(Value::Str(core_runtime::format_bin(req_int(0, "core.format_bin")?)))
            }
            "format_ptr" => {
                if args.len() != 1 {
                    return Err(self.err("core.format_ptr() takes exactly one argument"));
                }
                Ok(Value::Str(core_runtime::format_ptr(req_int(0, "core.format_ptr")?)))
            }
            "list_new" => {
                if args.len() > 1 {
            ...
            "list_len" => {
                ...
                Ok(Value::Int(req_list(0, "core.list_len")?.borrow().len() as i64))
            }
            "list_push" => {
                if args.len() != 2 {
            ...
            "list_pop" => {
                ...
                let value = list.borrow_mut().pop().unwrap_or(Value::Nil);
                Ok(value)
            }
            "dict_new" => {
                if !args.is_empty() {
                ...
                Ok(Value::Dict(Rc::new(RefCell::new(IndexedMap::new()))))
            }
            "dict_len" => {
                if args.len() != 1 {
                    return Err(self.err("core.dict_len() takes exactly one argument"));
                }
                Ok(Value::Int(req_dict(0, "core.dict_len")?.borrow().keys.len() as i64))
            }
            "dict_has" => {
                if args.len() != 2 {
